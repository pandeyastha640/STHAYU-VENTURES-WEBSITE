...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative isolate w-full overflow-hidden px-5 pb-16 pt-10 sm:px-6 lg:px-8 lg:pb-24 lg:pt-14"
    >
      <style>{`
        .sthayu-hero-field { position:absolute; inset:0; overflow:hidden; pointer-events:none; }
        .sthayu-orb { position:absolute; border-radius:9999px; border:1px solid rgba(103,232,249,.12); box-shadow:inset 0 0 35px rgba(34,211,238,.025),0 0 35px rgba(34,211,238,.025); animation: sthayuOrbit 18s linear infinite; }
        .sthayu-orb::before,.sth ayu-orb::after { content:""; position:absolute; border-radius:9999px; background:rgba(103,232,249,.8); box-shadow:0 0 14px rgba(103,232,249,.9); }
        .sth ayu-orb::before { width:4px;height:4px;left:14%;top:18%; }
        .sth ayu-orb::after { width:3px;height:3px;right:16%;bottom:20%;background:rgba(96,165,250,.85); }
        .sth ayu-orb.o1 { width:420px;height:420px;left:-120px;top:8%;animation-duration:24s; }
        .sth ayu-orb.o2 { width:560px;height:560px;right:-190px;top:18%;animation-duration:30s;animation-direction:reverse; }
        .sth ayu-orb.o3 { width:300px;height:300px;left:42%;bottom:-150px;animation-duration:20s; }
        .sth ayu-flow { position:absolute; height:1px; background:linear-gradient(90deg,transparent,rgba(103,232,249,.18),transparent); transform-origin:left center; animation:sth ayuFlow 7s ease-in-out infinite; }
        .sth ayu-flow.f1 { width:38%;left:2%;top:31%;transform:rotate(11deg); }
        .sth ayu-flow.f2 { width:34%;right:3%;top:43%;transform:rotate(-14deg);animation-delay:-2s; }
        .sth ayu-flow.f3 { width:42%;left:29%;top:74%;transform:rotate(-5deg);animation-delay:-4s; }
        .sth ayu-particle { position:absolute;width:3px;height:3px;border-radius:9999px;background:#67e8f9;box-shadow:0 0 12px rgba(103,232,249,.9);animation:sth ayuParticle 8s ease-in-out infinite; }
        .sth ayu-particle.p1 {left:17%;top:23%;}.sth ayu-particle.p2 {left:73%;top:18%;animation-delay:-2s}.sth ayu-particle.p3 {left:83%;top:69%;animation-delay:-4s}.sth ayu-particle.p4 {left:31%;top:82%;animation-delay:-6s}
        @keyframes sth ayuOrbit { to { transform:rotate(360deg); } }
        @keyframes sth ayuFlow { 0%,100%{opacity:.15;transform:translateX(-8px) rotate(var(--r,0deg))} 50%{opacity:.7;transform:translateX(20px) rotate(var(--r,0deg))} }
        @keyframes sth ayuParticle { 0%,100%{transform:translate3d(0,0,0);opacity:.25} 50%{transform:translate3d(20px,-24px,0);opacity:1} }
        .hero-ai-visual { position:relative;width:100%;height:clamp(500px,48vw,650px);overflow:hidden;border-radius:34px;border:1px solid rgba(103,232,249,.14);background:radial-gradient(circle at 50% 46%,rgba(34,211,238,.12),transparent 25%),radial-gradient(circle at 72% 22%,rgba(59,130,246,.10),transparent 28%),linear-gradient(145deg,#061a2c 0%,#030c17 72%,#020812 100%);box-shadow:0 45px 110px rgba(0,0,0,.48),inset 0 1px 0 rgba(255,255,255,.04);}
        ...
        .hero-ai-lines {position:absolute;inset:0;width:100%;height:100%;}
        .hero-flow-line {stroke-dasharray:7 12;animation:heroFlow 2.6s linear infinite;}
        .hero-flow-line.d1{animation-delay:-.8s}.hero-flow-line.d2{animation-delay:-1.5s}
        .hero-node{animation:heroNode 3s ease-in-out infinite;transform-box:fill-box;transform-origin:center}.hero-node.d1{animation-delay:-.7s}.hero-node.d2{animation-delay:-1.4s}.hero-node.d3{animation-delay:-2.1s}
        .hero-pulse-ring{animation:heroPulse 3.5s ease-out infinite;transform-origin:center;transform-box:fill-box}.hero-pulse-ring.d1{animation-delay:1.2s}
        .hero-core-shell{animation:heroCore 7s ease-in-out infinite;transform-origin:center}
        .hero-dashboard{animation:heroDashboard 5s ease-in-out infinite}.hero-dashboard.d1{animation-delay:-2.5s}
        ...
        .hero-tech-badge{position:absolute;z-index:30;border:1px solid rgba(103,232,249,.14);background:rgba(3,16,29,.86);box-shadow:0 18px 45px rgba(0,0,0,.28);backdrop-filter:blur(16px);}
        @media(max-width:640px){.hero-ai-visual{height:500px;border-radius:26px}.hero-dashboard{transform:scale(.88);transform-origin:top left}.hero-dashboard.d1{transform-origin:top right}.hero-dashboard.bottom-card{transform-origin:bottom left}.hero-dashboard.bottom-card.d1{transform-origin:bottom right}}
      `}</style>

      <div className="sth ayu-hero-field" aria-hidden="true">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(34,211,238,.08),transparent_34%)]" />
        <div className="sth ayu-orb o1" />
        <div className="sth ayu-orb o2" />
        <div className="sth ayu-orb o3" />
        <div className="sth ayu-flow f1" />
        <div className="sth ayu-flow f2" />
        <div className="sth ayu-flow f3" />
        <span className="sth ayu-particle p1" /><span className="sth ayu-particle p2" /><span className="sth ayu-particle p3" /><span className="sth ayu-particle p4" />
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        <div className="grid items-start gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16 xl:gap-20">
          <div className="relative z-20 max-w-2xl lg:pt-8">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><a href="#assessment" className="group inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 px-6 py-3.5 text-[13px] font-bold text-slate-950 shadow-[0_18px_50px_rgba(34,211,238,0.18)] transition-all duration-300 hover:-translate-y-1">Book Discovery Call<ArrowRight size={16} className="transition-transform duration-300 group-hover:translate-x-1" /></a><a href="#services" className="group inline-flex items-center justify-center gap-2 rounded-full border border-cyan-300/20 bg-slate-950/45 px-6 py-3.5 text-[13px] font-semibold text-white backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-cyan-300/35">Explore Solutions<ArrowUpRight size={16} /></a></div>
            <div className="mt-9 grid max-w-xl grid-cols-3 gap-2.5 sm:gap-3">{metrics.map(([label,value]) => <div key={label} className="rounded-2xl border border-cyan-300/10 bg-slate-950/40 px-3 py-3.5 backdrop-blur-xl"><div className="text-[9px] uppercase tracking-[0.16em] text-slate-400">{label}</div><div className="mt-1 text-base font-bold tracking-[-0.04em] text-white">{value}</div></div>)}</div>
          </div>
          ...
          <div className="relative z-10 flex min-w-0 justify-center lg:justify-end">
            <div className="relative w-full max-w-[650px]" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{transform:`perspective(1400px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`}}>
              ...
              <div className="hero-ai-visual">
                ...
                <div className="hero-tech-badge left-5 top-1/2 z-40 -translate-y-1/2 rounded-full px-3 py-2 text-[8px] font-semibold uppercase tracking-[.18em] text-cyan-200 sm:left-7"><span className="mr-2 inline-block h-1.5 w-1.5 rounded-full bg-cyan-300 shadow-[0_0_10px_rgba(103,232,249,.9)]"/>Live intelligence</div>
                ...
                {/* Reserved footer zone. This is deliberately a normal-flow block, never an absolute overlay. */}
                <div className="absolute bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-[#041321]/90 px-4 py-3 backdrop-blur-xl sm:px-6"><div className="flex min-w-0 items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-cyan-300/15 bg-cyan-300/[0.07] text-cyan-300"><Workflow size={14}/></div><div className="min-w-0"><p className="text-[10px] font-semibold text-white sm:text-[11px]">Connected business system</p><p className="mt-0.5 truncate text-[8px] text-slate-500 sm:text-[9px]">Leads → AI → CRM → Follow-up</p></div></div><div className="hidden items-center gap-2 text-[8px] uppercase tracking-[.16em] text-slate-500 sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_9px_rgba(110,231,183,.8)]"/>Running</div></div></div>
              </div>

              {/* These cards are in normal flow below the visual, so they can never overlap it. */}
              <div className="mt-4 grid w-full grid-cols-2 gap-3 sm:mt-5 sm:gap-4"><MiniValueCard label="Your team" title="Focus on decisions" variant="cyan"/><MiniValueCard label="Sthayu" title="Runs repetitive work" variant="blue"/></div>
            </div>
          </div>
        </div>
...
function MiniValueCard({ label, title, variant = "cyan" }) {
